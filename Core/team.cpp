...
Team::Team(QObject *parent, int CarIDX, Track *reftrack) : QObject(parent)
{
 ...
 //   this->avg_calculator = new Team_avg_calculator(nullptr, this);
 //   this->calc_thread = new QThread;
 //   this->calc_thread->setObjectName("Calc_Thread");

 //   this->avg_calculator->moveToThread(this->calc_thread);
   // connect(this->avg_calculator, SIGNAL(error(QString)), this, SLOT(errorString(QString)));
 //   connect(calc_thread, SIGNAL(started()), this->avg_calculator, SLOT(process()));
 //   connect(calc_thread, SIGNAL(finished()), calc_thread, SLOT(quit()));
  //  connect(calc_thread, SIGNAL(finished()), this->avg_calculator, SLOT(deleteLater()));
  //  connect(calc_thread, SIGNAL(finished()), calc_thread, SLOT(deleteLater()));
  //  connect(this, SIGNAL(do_avg_calculation()), avg_calculator, SLOT(do_work()));
  //  connect(this->avg_calculator, SIGNAL(new_data(Lap*, Lap*, Lap*, Lap*)), this, SLOT(new_avg_data(Lap*, Lap*, Lap*, Lap*)));
  //  this->calc_thread_started = false;
}
...
void Team::update_from_telemetry(ir_tel_vars *v){
  ...
  int interval = this->lap_dct * this->gl->interval_count;
  if((interval >= 0) && (interval <= this->gl->interval_count)){
      if(v->SessionTime->value[0].toFloat() > 0){
      this->Laps[this->curlap]->intervals[interval]->set_sessiontime(v->SessionTime->value[0].toFloat());
      this->Laps[this->curlap]->intervals[interval]->set_tracktype(this->TrackSurface);
      }
    }
...
//  if(this->calc_thread_started){
//  l.processEvents();
//  }else{
//      this->calc_thread->start();
//      this->calc_thread_started = true;
 // }
...
void Team::set_lap(SessionInfo_ResultPosition *r){
  ...
  if(completed != -1){
      ...
      if(this->Laps[completed]->set_laptime(last_laptime)){
          this->calculate_avg_2();
        }

  }
  if(bestlap != -1){
      if(bestlap < this->Laps.count()){
...
void Team::calculate_avg(){
  ...
  if(count > 0){
      ...
      this->avg_stint_lap->set_laptime(0);
  }
...
void Team::calculate_avg_2(){
    ...
    for(int y = 0; y < this->gl->interval_count +1; y++){
        count_interval.push_back(0);
        count_results.push_back(0.0f);
      }
    ...
    while(i > 0){
        i = i -1;
         if((this->Laps[i]->laptime > 0)){
        if(count_5 < x_5){
                avg_5 = avg_5 + this->Laps[i]->laptime;
                count_5++;
            }
        if(count_10 < x_10){
            avg_10 = avg_10 + this->Laps[i]->laptime;
            count_10++;
        }
        if(count_20 < x_20){
            avg_20 = avg_20 + (this->Laps[i]->laptime);
            count_20++;
        }
        if(count_40 < x_40){
            avg_40 = avg_40 + (this->Laps[i]->laptime * count_40_x);
            if(this->Laps[i]->laptype != Lap::l_pit){
            for(int y = 0; y < this->Laps[i]->intervals.count(); y++){
                if(calc_sessiontime == 0.0f){
                  }else{
                    if((this->Laps[i]->intervals[y]->sessiontime > 0) && (this->Laps[i]->intervals[y]->tracklok == irsdk_OnTrack)){
                        float diff = this->Laps[i]->intervals[y]->sessiontime - calc_sessiontime;
                        if((diff > 0) && (diff < 10)){
                 ...
                 calc_sessiontime = this->Laps[i]->intervals[y]->sessiontime;
              }
            }else{
    ...
    for(int i = 0; i < this->gl->interval_count; i++){
        if(count_results[i] > 0){
            this->avg_40_lap->intervals[i]->sessiontime = count_interval[i] / count_results[i];
          }else{
            this->avg_40_lap->intervals[i]->sessiontime = 0;
          }
      }     
}
...
void Team::calc_gaps(Teams *teams){
  ...
  for(int i = 0; i < teams->teams.count(); i++){
      ...
      float diff =   t->distance_driven - this->distance_driven;
      if(diff > 5){
          this->Gaps[i]->ok = true;
          this->Gaps[i]->gap = diff;
          this->Gaps[i]->time_mode = false;
        }else if(diff < 0){
          this->Gaps[i]->ok = false;
        }else{
      int calc_distance = this->distance_driven * this->gl->interval_count;
      int calc_distance_2 = t->distance_driven * this->gl->interval_count;
      float gap = 0.0f;
      while(calc_distance <= calc_distance_2){
          int tmp_distance = calc_distance;
          while(tmp_distance > this->gl->interval_count){
              tmp_distance = tmp_distance - this->gl->interval_count;
            }
          if((tmp_distance >=0) && (tmp_distance < this->gl->interval_count)){
          gap = gap + this->avg_40_lap->intervals[tmp_distance]->sessiontime;
          }
          calc_distance = calc_distance + 1;
        }
      this->Gaps[i]->gap = gap;
      this->Gaps[i]->ok = true;
      this->Gaps[i]->time_mode = true;
        }
...
void Team::reset(){
 ...
 this->current_sector = -1;
  ...
  this->fastestlap = -1;
  //this->avgxlap->laptime = 0;
  //this->avg_laps->laptime = 0;
  //this->avg_clean_lap->laptime = 0;
  //this->avg_stint_lap->laptime = 0;
    this->avg_5_lap = new Lap(this);
  // this->avg_10_lap  = new Lap(this);
  //  this->avg_20_lap = new Lap(this);
    this->avg_40_lap = new Lap(this);
  ...
  this->offtrackcounter = 0;
    ...
    this->end_lap = 0;
    this->end_sessiontime = 0;


  while(this->Gaps.count() < 64){
      Gap *g = new Gap(this);
      this->Gaps.push_back(g);
    }
...
void Team::new_avg_data(Lap *avg_5_lap, Lap *avg_10_lap, Lap *avg_20_lap, Lap *avg_40_lap){
    this->avg_5_lap = avg_5_lap;
    this->avg_10_lap = avg_10_lap;
    this->avg_20_lap = avg_20_lap;
    this->avg_40_lap = avg_40_lap;
}
